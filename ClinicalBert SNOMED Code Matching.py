...
# The first strategy assigns the entity label of the first sub-token (di) to the entire word. Alternatives are max, average, simple. Simple splits tokens like hypertension into hyper and tension.
nlp_ner = pipeline("ner", model=model, tokenizer=tokenizer, aggregation_strategy="max", device=device) 
...
#remove entities where confidence score is lower than 0.9 to reduce inaccuracies
filtered_entities_with_high_confidence = [entity for entity in entities if float(entity['score']) >= 0.9]

#remove entities where length of the term is less than 3 - must have more than 3 characters for a good match
filtered_entities_with_length_longer_than_3 = [entity for entity in filtered_entities_with_high_confidence if len(entity["word"]) > 3]
...
for entity in filtered_entities_with_length_longer_than_3:
